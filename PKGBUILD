...
arch=('x86_64')
license=('PHP')
depends=('php7' 'imagemagick' 'ttf-font')
checkdepends=('librsvg' 'ttf-dejavu')
backup=("etc/php7/conf.d/30-$app_name.ini")
...
check() {
  ...
  export TEST_PHPDBG_EXECUTABLE="/usr/bin/phpdbg7"
  make -k test
}
...
package() {
  ...
  install -vDm 644 ${ini_name} -t "$pkgdir/etc/php7/conf.d/"
